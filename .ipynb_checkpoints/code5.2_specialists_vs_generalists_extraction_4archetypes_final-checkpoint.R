...
# Run the Python script and capture its output. Please mention the sample name in the python script "code5.2_distance_cutoff_automate_main_4archetypes.py" before running this R script
output <- system2("python", args = c("../../pipeline_testing/codes/code5.2_distance_cutoff_automate_main_4archetypes.py"), stdout = TRUE)
...
# Please put your sample names here and this name should match with the sample name you provided in "code5.2_distance_cutoff_automate_main_4archetypes.py"
sample_name <- "SMC25-T"   
...
all_specialists<-list(specialist_a1_gene_exp,specialist_a2_gene_exp,specialist_a3_gene_exp,specialist_a4_gene_exp)
...
all_specialists_gene_exp <- cbind(all_specialists[[1]],all_specialists[[2]],all_specialists[[3]],all_specialists[[4]])
...
heatmap.2(matrix_heatmap,trace="none",Colv=FALSE,Rowv=TRUE,dendrogram='none',cexRow=1,cexCol=0.5,keysize=0.8,col=bluered(256), margins=c(25,15),ColSideColors=c(rep("royalblue",ncol(all_specialists[[1]])), rep("cyan",ncol(all_specialists[[2]])),rep("olivedrab",ncol(all_specialists[[3]])),rep("seagreen",ncol(all_specialists[[4]])), rep("rosybrown",ncol(generalists_gene_exp))),scale='row',breaks=seq(-2.5,2.5,length.out=257))
legend("topright",legend=c("Archetype 1","Archetype 2","Archetype 3","Archetype 4", "Generalists"),fill=c("royalblue", "cyan", "olivedrab", "seagreen", "rosybrown"),border=FALSE, bty="n", y.intersp = 1.5, cex=3.5)
dev.off()